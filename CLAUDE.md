...
## - curl -H "Content-Type: text/plain" -d "${FEATURE} 배포완료! ${DEPLOY_URL}" "https://ntfy.sh/${PWD##*/}"
...
- 기능 1개 report 완료되면:
  ...
  6. 모든 기능 [x]이면 (마지막 기능):
     ...
     - curl -H "Content-Type: text/plain" -d "${FEATURE} 완료! 배포: ${DEPLOY_URL}" "https://ntfy.sh/${PWD##*/}"
     - curl -H "Content-Type: text/plain" -d "${PWD##*/} 전체 완료! 🎉" "https://ntfy.sh/${PWD##*/}"
     - "전체완료!" 출력 후 종료
  7. echo "${FEATURE} clear 시도" >> clear_log.txt
  8. /clear 실행
  9. /context >> context_after_clear.txt
  10. sleep 3
  11. curl -H "Content-Type: text/plain" -d @context_after_clear.txt "https://ntfy.sh/${PWD##*/}"
  12. echo "${FEATURE} clear 성공 확인" >> clear_log.txt
  13. (새 세션 시작시) echo "새 세션 시작 - 다음기능: ${NEXT_FEATURE}" >> clear_log.txt
  14. curl -H "Content-Type: text/plain" -d "새 세션 시작 - 다음기능: ${NEXT_FEATURE}" "https://ntfy.sh/${PWD##*/}"
...
- 에러 수정 완료되면:
  ...
  2. git push
  3. curl -H "Content-Type: text/plain" -d "${FEATURE} 에러 발생! ERROR.md 확인" "https://ntfy.sh/${PWD##*/}"
